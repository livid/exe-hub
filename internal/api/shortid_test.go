...
// and came up 404. Codex's fixture: those eight stay a 404, under the
// floor, and the first twelve with ?lang=zh are the positive case.
const (
...
func TestWebShortIDFixture(t *testing.T) {
	...
	for _, path := range []string{sixColumnsShort, "/p/" + sixColumns} { // the post is not here yet: short or whole, a 404 that says so for now only
	...
	if w := ask(sixColumnsCut); w.Code != http.StatusNotFound || w.Header().Get("Location") != "" {
		t.Errorf("GET %s = %d to %q, want 404: eight characters are under the floor", sixColumnsCut, w.Code, w.Header().Get("Location"))
	}
